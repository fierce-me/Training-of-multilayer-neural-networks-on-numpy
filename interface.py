...
class Layer(abc.ABC):
    def __init__(self, input_shape=None):
        if not isinstance(input_shape, (tuple, type(None))):
            raise RuntimeError("input_shape must be a tuple of integers.")
    ...
    def build(self, optimizer, prev_layer=None):
        ...
        self.output_shape = (
            self.input_shape if self.output_shape is None else self.output_shape
        )
        self._is_built = True
    ...
    def add_parameter(self, name, shape, initializer):
        ...
        self._parameter_updaters[name] = self._optimizer.get_parameter_updater(shape)
    ...
    def num_parameters(self):
        return sum(getattr(self, name).size for name in self._parameter_updaters.keys())
...
class SkipConnection(Layer):
    def __init__(self, layers, *args, **kwargs):
        ...
        if (
            ...
            or not len(layers)
            or not all(isinstance(l, Layer) for l in layers)
        ):
            raise ValueError(
...
def convolve_pytorch(inputs, kernels, padding):
    ...
    with warnings.catch_warnings():
        ...
        warnings.filterwarnings(
            ...
            message=(
                r"The given NumPy array is not write?able, and PyTorch does "
